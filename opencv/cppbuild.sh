...
case $PLATFORM in
    ...
    android-x86)
        ...
        ;;
    linux-arm)
        cmake -DCMAKE_INSTALL_PREFIX=.. -DBUILD_PYTHON_SUPPORT=OFF -DBUILD_SHARED_LIBS=ON -DBUILD_EXAMPLES=ON -DBUILD_TESTS=OFF -DBUILD_PERF_TESTS=OFF -DBUILD_JASPER=ON -DBUILD_JPEG=ON -DBUILD_OPENEXR=ON -DBUILD_PNG=ON -DBUILD_TBB=ON -DBUILD_TIFF=ON -DBUILD_ZLIB=ON -DBUILD_opencv_java=OFF -DBUILD_opencv_python=OFF -DENABLE_PRECOMPILED_HEADERS=OFF -DWITH_1394=OFF -DWITH_FFMPEG=ON -DWITH_GSTREAMER=OFF -DWITH_TBB=ON -DWITH_CUDA=OFF -DWITH_OPENCL=OFF -DWITH_V4L=ON -DWITH_GTK=ON -DWITH_UNICAP=ON
        make
        make install/strip
        ;;
    linux-x86)
